...
int main (int argc, char* args[]){
   ...
   SDL_Init(SDL_INIT_EVERYTHING); //iniciar o sdl
    ...
    //para o tamanho do inimigo e posição inicial
	float inimigoX = 50;
	float inimigoY = 60;
	float inimigoCA = 30;
	...
	while(executando){
        ...
        if(inimigoY+inimigoCA>600)
            velY = -velY;
            // se quiser para quando bater no chao perder, pôr
            // executando = false;
        ...
        glColor4ub(255,255,255,255); // branco para não alterar a cor
        ...
        for(int i=0; i<BLOCOS;i++){
            if(bloco[i].vida==true){ // condição para desenhar o bloco
...
bool colisao(float Ax, float Ay,float Acomp, float Aalt,float Bx, float By,float Bcomp, float Balt){
    ...
    return true; // quando é igual tem a colisão
}
